...
void main(void) {
  ...
  uv = uv * vec2( clamp(sqrt(2.)+dot(uv,uv)/2.,.0,1.) );
  ...
  color = vec4(hsl2rgb( vec3(
  	...
  	(hsl.g + 12.)* .025 , // + 2.*sin(time/3.) , //.6
  	hsl.b * 1.2) //+ .3
  	), 1.);

  color = 0.5 + ( (1./2. * normalize(mix(color, vec4(.5), .5)) ) - vec4(1./2.));

  vec4 m = (vec4( vec3(color.r + color.g + color.b)*.75, 1.0));
  color = heat(color);

  vec4 msk = vec4(1.-pow(dot(uv,uv), 0.85373472095314 ));

  gl_FragColor = pow(msk * m * color*color,vec4(1./1.8)) ;

/*
  if(gl_FragCoord.x < resolution.x / 2.){
    gl_FragColor = m ; //color
  }else if(gl_FragCoord.x > resolution.x / 2. &&
  				 gl_FragCoord.y < resolution.y / 2.){
      gl_FragColor = (m * color) ; //color
  }
*/
}
