...
void main(void) {
	...
	noise = pnoise(p + 48.*vec3(uv, pow(0.85373472095314, 2.)+pow(uv.x*uv.x + uv.y*uv.y, 1.79284291400159)), (3.)*vec3( 1., 1., -1.));
	float m =  1.-clamp( pow(distance(uv,vec2(.0,.0)), 1.) , .0, 1.);

	c1 = vec4(m*dot(vec2(noise+clamp(ms*uv,.0,1.)),vec2(m+clamp(ms*uv,.0,1.)) ));
	c2 = vec4(c1*dot(vec2(noise+clamp(ms*uv,.0,1.)),vec2(+clamp(ms*uv,.0,1.)) ));
	color = c1 + c2 ;
