...
void main(void) {
	...
	p3 = vec2(.0, .125) + .5*vec2(cos(5.*(t+4.*pi/3.)), sin(7.*(t+4.*pi/3.)));
  ...
  txz  = step(.75, s3) * .8 * s3 * texture(brushcut, 8./4.*s3*((txc+1./16.*p3)-3. /12. + (3./12.*ms))/length(s3) );
	txz += step(.75, s3) * .7 * s3 * texture(brushcut, 6./4.*s3*((txc+2./16.*p3)+14./12. + (2./12.*ms))/length(s3) );
	txz += step(.75, s3) * .6 * s3 * texture(brushcut, 3./4.*s3*((txc+3./16.*p3)+2. /12. - (2./12.*ms))/length(s3) );
	txz += step(.75, s3) * .5 * s3 * texture(brushcut, 2./4.*s3*((txc+6./16.*p3)+6. /12. - (6./12.*ms))/length(s3) );
	txz += step(.75, s3) * .4 * s3 * texture(brushcut, 1./4.*s3*((txc+8./16.*p3)+18./12. -(18./12.*ms))/length(s3) );
	txz += 2.*step(.75, s3)  * texture(brushcut, (txc-5.+.5*p3)/length(3.*s3) );

	txz /= 5.;
	txz = step(.75, s3) * pow(txz + .375, vec4(3.));
