...
float voronoi2D(in vec2 n)
{
    ...
    return pow(1.0 - dis/2., exp(1.));// + ERRMN;
}
...
vec3 GetColor(vec2 p)
{
    ...
    col = mix( vec3(0.78, 0.45, 0.06), vec3(0.35, .0, .3), f );
    col = mix( col, vec3( 0.81, 0.55, .0), dot(on.xy, on.zw));
...
void main(void)
{
    ...
    if ( dot(p, p) < sz*sz) {
    	col = GetColor(p - 10.*vec2(cos(3.*time/50.),sin(time/50.))) * ( sz*sz - dot(p, p) )/sz;
    ...
    col = mix( 2. * col, tx.rgb*.95, .97);//col-.0125
    // Output to screen.
